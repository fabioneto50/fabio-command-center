   ...
   for w,h in [(390,844),(1440,1000)]:
    ...
    try:
     ...
     for theme in ['light','dark']:
      ...
      last=pager.locator('button').last;number=last.inner_text();last.click()
      p.wait_for_function("(n)=>document.querySelector('#homeHealthNewsList [aria-current=page]')?.textContent.trim()===n",arg=number)
      check(tag+' '+theme+' numbered page responds',pager.locator('[aria-current=page]').inner_text()==number)
      area(p,'clinical')
