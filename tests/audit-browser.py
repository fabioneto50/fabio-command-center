...
def test_context(pw,name,width,height):
 ...
 try:
  ...
  page.evaluate('FCCAccess.lock()');check(prefix+' lock removes private DOM',page.evaluate("!document.body.textContent.includes('SYNTHETIC SECRET')&&!FCCAccess.isUnlocked()"));check(prefix+' private route cannot bypass guard',page.evaluate("async()=>{await fccNavigate('expenses',{bypassGuard:true});return !FCCAccess.isPrivate(FCCNavigation.current())&&!FCCAccess.isUnlocked();}"));page.keyboard.press('Escape')
