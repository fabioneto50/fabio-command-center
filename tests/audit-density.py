...
Measures all preparation records and all catalogue fichas at 1440x900, and checks
real interactions, text preservation and layout in both engines/themes/sizes.
"""
...
report={'commit':os.environ.get('GITHUB_SHA','local'),'build':BUILD,'live':LIVE,'checks':[],'measurements':[],'coverage':[], 'limits':'CSS pixels in software engines; not physical iPhone. Single screen includes all clinical fields but not an expanded reference disclosure.'}
    ...
    try:
     ...
     for theme in ['light','dark']:
      ...
      for q in ['Amiodarona','amoxicilina','ceftriaxona','noradrenalina']:
       p.locator('#perfDilutionSearch').fill(q);p.wait_for_timeout(400)
       ...
       p.locator('#fccDilutionBack').click();check(t+' return preserves search',p.locator('#perfDilutionSearch').input_value()==q)
      ...
      if width>=1000:
       check(tag+theme+' catalogue uses available columns',p.locator('.fcc-facts-grid').evaluate("e=>parseFloat(getComputedStyle(e).columnWidth)>=260&&e.clientWidth>850"))
     ...
     # Measure every record without collapsing any clinical section to fake a one-screen result.
     if width==1440 and not LIVE:
