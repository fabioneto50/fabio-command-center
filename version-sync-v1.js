(()=>{
  ...
  function version(){return String(window.FCC_RUNTIME_VERSION||'1.1.0')}
  function sync(){
    const v=version();
    document.querySelectorAll('.master-chip').forEach(el=>el.textContent='MASTER '+v);
    ...
    if(page){
      ...
      if(!box){
        ...
        box.innerHTML='<div class="row"><div><span class="eyebrow">APLICAÇÃO</span><h3 style="margin:2px 0">Versão instalada</h3><p>Versão real do runtime/PWA carregado neste dispositivo.</p></div><div class="spacer"></div><span class="badge good" id="fccRuntimeVersionBadge"></span></div>';
        const head=page.querySelector(':scope > .pagehead');const grid=page.querySelector(':scope > .grid');
  ...
  const old=window.renderMaintenance;
  if(typeof old==='function')window.renderMaintenance=function(...args){const r=old.apply(this,args);setTimeout(sync,0);return r};
