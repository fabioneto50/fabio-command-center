(()=>{
  if(window.__fccRuntimeDiagnosticsV1Installed)return;
  window.__fccRuntimeDiagnosticsV1Installed=true;
  ...
  function module(name,ok,detail=''){log(ok?'module-ok':'module-error',name,detail)}
  ...
  function render(){
    ...
    if(!card){
      card=document.createElement('div');card.id='fccRuntimeDiagnosticsCard';card.className='card full';
      const grid=page.querySelector(':scope > .grid');if(grid)grid.appendChild(card);else page.appendChild(card);
    }
    const errors=rows.filter(x=>x.type.includes('error')||x.type==='window-error'||x.type==='promise-error').slice(-12).reverse();
    card.innerHTML=`<div class="row"><div><span class="eyebrow">DIAGNÓSTICO</span><h3 style="margin:2px 0">Runtime / estabilidade</h3><p>Regista falhas JavaScript e de carregamento neste dispositivo, sem enviar dados para fora do browser.</p></div><div class="spacer"></div><span class="badge ${errors.length?'warn':'good'}">${errors.length?errors.length+' falhas recentes':'Sem falhas registadas'}</span></div><div class="actions"><button type="button" class="btn small" id="fccDiagRefresh">Atualizar</button><button type="button" class="btn small" id="fccDiagClear">Limpar registo</button></div><div class="list" style="margin-top:8px">${errors.length?errors.map(x=>`<div class="item"><div><strong>${esc(x.type)}</strong><br><span>${esc(x.message)}</span>${x.extra?`<br><span>${esc(x.extra)}</span>`:''}</div><span class="tiny">${esc(new Date(x.at).toLocaleTimeString('pt-PT',{hour:'2-digit',minute:'2-digit'}))}</span></div>`).join(''):'<div class="item"><span>Nenhum erro JavaScript recente foi registado.</span></div>'}</div>`;
    card.querySelector('#fccDiagRefresh')?.addEventListener('click',render,{once:true});
    card.querySelector('#fccDiagClear')?.addEventListener('click',()=>{rows=[];persist();render()},{once:true});
  }
  ...
  document.addEventListener('fcc-page-change',e=>{if(e.detail?.page==='settings')queueMicrotask(render)});
  window.addEventListener('pageshow',()=>queueMicrotask(render));
  window.FCCDiagnostics={log,module,render,get:()=>rows.slice(),clear:()=>{rows=[];persist();render()}};
})();
