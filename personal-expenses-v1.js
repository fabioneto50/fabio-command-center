(()=>{
  ...
  function card(){
    ...
    const labels=[...page.querySelectorAll(':scope > .tabs > .tab')].map(x=>x.textContent.trim()).filter(Boolean),b=document.createElement('button');b.className='personal-area';b.type='button';b.dataset.personalPage='expenses';b.innerHTML=`<span class="personal-area-count">${labels.length} subtópicos</span><div class="personal-area-head"><span class="personal-area-code">€</span><div><h3>Despesas</h3><p>Registo, análise, orçamentos, recorrentes e relatórios.</p></div></div><div class="personal-mini-tabs">${labels.slice(0,5).map(x=>`<span>${String(x).replace(/[&<>"']/g,'')}</span>`).join('')}${labels.length>5?`<span>+${labels.length-5}</span>`:''}</div>`;b.addEventListener('click',()=>window.openCategoryMenu?.('expenses')||window.fccNavigate?.('expenses'));grid.appendChild(b);return true
  }
