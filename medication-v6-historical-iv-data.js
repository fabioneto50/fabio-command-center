(()=>{
  if(window.__fccMedicationV6HistoricalIVDataInstalled)return;
  window.__fccMedicationV6HistoricalIVDataInstalled=true;
  const EXPECTED=198;
  const HISTORICAL=[
  ...
  window.FCCMedicationV6HistoricalIVNames=Object.freeze(UNIQUE.slice());
  window.FCCMedicationV6HistoricalIVSet=new Set(UNIQUE.map(x=>String(x).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/\s+/g,' ').trim()));
  window.FCCMedicationV6HistoricalIVHealth={ok,expected:EXPECTED,count:UNIQUE.length,source:'iv-catalogue.js @ V0.2 commit 0264d930'};
  if(!ok)console.error(`[Medication V6 historical IV] integrity failed: ${UNIQUE.length}/${EXPECTED}`);
})();
