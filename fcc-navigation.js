...
(()=>{
 ...
 function cleanRoute(value){
  const p=String(value||'').replace(/^#\/?/,'').split('/').map(x=>{try{return decodeURIComponent(x);}catch{return '';}});
  const page=Object.hasOwn(labels,p[0])?p[0]:'home',sub=p[1]||'',ref=p[2]||'';
  return {page,sub:/^[a-z0-9-]{0,100}$/.test(sub)?sub:'',ref:!isPrivate(page)&&/^[a-zA-Z0-9_.-]{0,200}$/.test(ref)?ref:''};
 }
 function hash(route){return '#/'+[route.page,route.sub,route.ref].filter(Boolean).map(encodeURIComponent).join('/');}
 ...
 async function navigate(page,opts={}){
  ...
  try{
   ...
   if(opts.ref){if(opts.sub==='clin-drugs')FCCMedications.show(opts.ref,{route:false});else if(opts.sub==='clin-cases')FCCCases.open(opts.ref,{route:false});}
 ...
 window.FCCNavigation=Object.freeze({navigate,addGuard:fn=>{if(typeof fn==='function')guards.push(fn);},removeGuard:fn=>{const i=guards.indexOf(fn);if(i>=0)guards.splice(i,1);},setParent:(page,parent)=>parents.set(page,parent),current:()=>currentRoute.page,route:()=>({...currentRoute}),replaceDetail,closeTransient,items,labels,restore:()=>navigate(cleanRoute(location.hash).page,{...cleanRoute(location.hash),history:true}),rebind});
