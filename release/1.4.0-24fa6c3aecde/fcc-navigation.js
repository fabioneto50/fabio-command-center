...
(()=>{
 ...
 const labels={home:'Início',clinical:'Área clínica',personal:'Pessoal',emergency:'Emergência',comms:'Comunicações',garage:'Veículos',research:'Investigação',expenses:'Despesas',settings:'Definições',favorites:'Favoritos'};
 ...
 function items(page){
  let rows=page==='clinical'?clinical.map(([id,title,description])=>({id,title,description})):page==='personal'?FCCAreas.privateAreas.map(x=>({...x})):[];
 ...
 function dilutionFilters(input={}){return {q:String(input.q||'').slice(0,180),g:String(input.g||'').slice(0,160),v:input.v==='1'?'1':''};}
 function cleanRoute(value){
  ...
  if(page==='clinical'&&sub==='clin-perf')route.filters=dilutionFilters(Object.fromEntries(new URLSearchParams(search)));
  return route;
 }
 function hash(route){
  ...
  if(route.page==='clinical'&&route.sub==='clin-perf'){const f=dilutionFilters(route.filters),p=new URLSearchParams();if(f.q)p.set('q',f.q);if(f.g)p.set('g',f.g);if(f.v)p.set('v',f.v);if(p.size)value+='?'+p.toString();}
  return value;
 }
 ...
 async function navigate(page,opts={}){
  ...
  const publicOverview=page==='personal'&&!opts.sub&&!opts.ref;
  if(isPrivate(page)&&!publicOverview&&!FCCAccess.isUnlocked())return FCCAccess.run(()=>navigate(page,opts));
  ...
  const my=++serial;positions.set(hash(currentRoute),window.scrollY);closeTransient();window.FCCDilutionView?.leave();
  ...
  if(page==='clinical'&&opts.sub==='clin-perf')currentRoute.filters=dilutionFilters(opts.filters);
  if(!opts.history)write(currentRoute,!!opts.replace);
  try{
   const group=publicOverview?'shell':FCCModules.forRoute(page,opts.sub||'');
   await FCCModules.ensure(group);
   if(my!==serial)return false;
   if(isPrivate(page)&&!publicOverview&&!FCCAccess.isUnlocked())return navigate('home',{replace:true});
   if(opts.sub)await activate(page,opts.sub,null,{route:false});
   else FCCAreas.show(page);
   if(opts.sub==='clin-drugs')FCCMedications.restoreRoute(currentRoute,{focus:!opts.history});
   else if(opts.sub==='clin-perf')await FCCDilutionView.restoreRoute(currentRoute,{focus:!opts.history});
   ...
   if(!U.active()){window.scrollTo({top:restore,behavior:'auto'});if(opts.focus){const heading=target.querySelector(currentRoute.ref?'#med4Results h3,.fcc-record-selected h3':opts.sub?'.fcc-area-current':'.pagehead h2');if(heading){heading.tabIndex=-1;heading.focus({preventScroll:true});}}}
 ...
 function replaceFilters(filters){if(currentRoute.page!=='clinical'||!['clin-drugs','clin-perf'].includes(currentRoute.sub))return;currentRoute={...currentRoute,ref:'',filters:currentRoute.sub==='clin-drugs'?medicationFilters(filters):dilutionFilters(filters)};write(currentRoute,true);}
 function replaceDetail(ref='',replace=true){positions.set(hash(currentRoute),window.scrollY);if(isPrivate(currentRoute.page))ref='';currentRoute={...currentRoute,ref};write(currentRoute,replace);return positions.get(hash(currentRoute))||0;}
 ...
 document.addEventListener('click',e=>{const nav=e.target.closest('.nav[data-page]');if(!nav)return;e.preventDefault();e.stopImmediatePropagation();nav.focus({preventScroll:true});const p=nav.dataset.page;navigate(p,{focus:true});},true);
