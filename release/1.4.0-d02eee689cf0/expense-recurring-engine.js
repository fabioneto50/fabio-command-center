(()=>{
  ...
  const parseDate=s=>{if(!FCCCore.validCivilDate(String(s)))throw new Error('Data civil inválida: '+s);const [y,m,d]=s.split('-').map(Number);return new Date(y,m-1,d,12);};
  ...
  function read(){
    try{
      const x=JSON.parse(FCCStore.getItem(KEY)||'null');
  ...
  function write(state,detail={}){FCCStore.setItem(KEY,JSON.stringify(state));emit(detail)}
  ...
  function normalizeRecurring(state){
    ...
    for(const x of state.expenses||[]){
      if(!x?.recurring||!x.merchant||out[x.merchant]||state.deletedPlans?.[fold(x.merchant)])continue;
  ...
  function mergeIncoming(value){
    const incoming=FCCCore.parseJSON(value),existing=read();if(!incoming||typeof incoming!=='object'||!Array.isArray(incoming.expenses))throw new Error('Dados de despesas inválidos.');
    incoming.deletedPlans={...(existing.deletedPlans||{}),...(incoming.deletedPlans||{})};incoming.skippedOccurrences={...(existing.skippedOccurrences||{}),...(incoming.skippedOccurrences||{})};
    incoming.recurring={...(existing.recurring||{}),...(incoming.recurring||{})};
    for(const k of Object.keys(incoming.recurring))if(incoming.deletedPlans[fold(k)])delete incoming.recurring[k];
    normalizeRecurring(incoming);return JSON.stringify(incoming);
  }

  function saveIncoming(state,detail={source:'expense-center'}){
    const next=mergeIncoming(JSON.stringify(state));
    FCCStore.setItem(KEY,next);
    emit(detail);
  }
  ...
  function hasMovement(state,plan,date){
    return !!state.skippedOccurrences?.[plan.id+'|'+date]||(state.expenses||[]).some(x=>x.recurringPlanId===plan.id&&x.date===date||x.recurring&&fold(x.merchant)===fold(plan.merchant)&&x.date===date&&Math.abs(Number(x.amount||0)-Number(plan.amount||0))<0.005);
  }
  function processDue(){if(!FCCStore.status().unlocked)return 0;
    ...
    else FCCStore.setItem(KEY,JSON.stringify(state));
    return added;
  }
  ...
  function savePlan(input){
    ...
    if(!merchant)throw new Error('Comerciante obrigatório');if(!FCCCore.validCivilDate(String(input.startDate)))throw new Error('Data de início inválida');FCCCore.number(input.amount,'valor',{min:0,exclusiveMin:true});state.deletedPlans=state.deletedPlans||{};delete state.deletedPlans[fold(merchant)];
    const current=state.recurring[oldKey]||state.recurring[merchant]||{};if(oldKey&&oldKey!==merchant){delete state.recurring[oldKey];state.deletedPlans[fold(oldKey)]=true;}
  ...
  function removePlan(key){const state=read();normalizeRecurring(state);state.deletedPlans=state.deletedPlans||{};state.deletedPlans[fold(key)]=true;delete state.recurring[key];write(state,{source:'recurring-ui',action:'remove-plan'});}
  function removeMovement(id){const state=read(),row=(state.expenses||[]).find(x=>x.id===id);if(row?.autoRecurring&&row.recurringPlanId){state.skippedOccurrences=state.skippedOccurrences||{};state.skippedOccurrences[row.recurringPlanId+'|'+row.date]=true;}state.expenses=(state.expenses||[]).filter(x=>x.id!==id);write(state,{source:'recurring-ui',action:'remove-movement'});}
  ...
  if(FCCStore.status().unlocked){const boot=read();normalizeRecurring(boot);FCCStore.setItem(KEY,JSON.stringify(boot));processDue();}
  window.FCCRecurringEngine={KEY,read,write,saveIncoming,normalizeRecurring,normalizePlan,occurrences,processDue,savePlan,setStatus,removePlan,removeMovement,nextOccurrence,monthStats,today,moneyEq};
})();