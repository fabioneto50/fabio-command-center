...
(()=>{
 ...
 const PAGE_SIZE=36;let records=[],page=0,query='',group='',current=null,brands={};
 ...
 function sourceLinks(record){
  const values=[record.sourceRegulatory,record.sourceClinical,record.src].flat().filter(Boolean),links=[];
 ...
 function shownRows(){const q=C.fold(query);return records.filter(r=>(!group||r.g===group)&&(!q||C.fold([r.n,r.g,r.q,r.routeVariant,...(brands[C.fold(r.n)]||[])].join(' ')).includes(q)));}
 function render(){
  ...
  const prev=root.querySelector('#fccMedPrev'),next=root.querySelector('#fccMedNext');if(prev)prev.onclick=()=>{page--;render();root.scrollIntoView({block:'start'});};if(next)next.onclick=()=>{page++;render();root.scrollIntoView({block:'start'});};
 }
 function detail(record){
  ...
  root.innerHTML=`<article class="med5-detail-row" data-name="${esc(record.n)}"><div class="med4-detail"><div class="fcc-detail-heading"><h3 tabindex="-1">${esc(record.n)}</h3><button class="btn" id="fccMedBack">Voltar aos resultados</button><button class="btn" id="fccMedFavorite" aria-label="Adicionar ${esc(record.n)} aos favoritos">Favorito</button></div><small>${esc(record.g||'')}</small>${brands[C.fold(record.n)]?.length?`<p>Nomes comerciais na base recebida: ${esc(brands[C.fold(record.n)].join(', '))}</p>`:''}<p>${esc(record.q||record.use||'Sem resumo nas fontes recebidas.')}</p>${record.risk?`<div class="fcc-detail-warning"><strong>Precauções</strong><p>${esc(record.risk)}</p></div>`:''}<div class="actions"><button class="btn" id="fccMedDilutions">Ver diluições</button><button class="btn" id="fccMedCompat">Ver compatibilidades</button></div>${FIELDS.filter(([k])=>record[k]||mandatory.has(k)).map(([k,label])=>`<section class="fcc-fact" data-clinical-field="${esc(k)}"><h4>${esc(label)}</h4><p>${esc(record[k]||'Sem informação nas fontes utilizadas.')}</p></section>`).join('')}<details class="fcc-provenance"><summary>Referências utilizadas e proveniência</summary><p>Dados consolidados da versão publicada em 16/09/2026. Esta atualização é de engenharia e não constitui uma nova revisão clínica individual das 923 fichas.</p><p>Revisão humana indicada na fonte: ${esc(record.humanReview||'não documentada')}. ${record.s?esc(record.s):''}</p><p>As referências abaixo foram recebidas com esta ficha. Não existe, na base original, uma ligação verificável de cada afirmação à página ou secção da fonte; não é atribuída uma validação que não foi realizada.</p><div class="fcc-reference-list">${sourceLinks(record)}</div><p>Confirmar formulação, via e protocolo aplicável. Informação não disponível não significa ausência de risco.</p></details></div></article>`;
  root.querySelector('#fccMedBack').onclick=()=>{current=null;render();FCCNavigation.replaceDetail('');document.getElementById('med4Search').focus();};
 ...
 function show(name,{route=true,focus=true}={}){
  ...
  current=record;detail(record);if(route)FCCNavigation.replaceDetail(id(record),false);if(focus)document.querySelector('#med4Results h3')?.focus({preventScroll:true});
 ...
 function install(){
  ...
  let timer;host.querySelector('#med4Search').oninput=e=>{query=e.target.value;page=0;current=null;clearTimeout(timer);timer=setTimeout(render,120);};
  host.querySelector('#fccMedGroup').onchange=e=>{group=e.target.value;page=0;current=null;render();};render();
 ...
 window.FCCMedicationReady=(async()=>{
  ...
  window.FCCMedications=Object.freeze({show,render,id,get:()=>records});
  install();return true;
 })();
})();
