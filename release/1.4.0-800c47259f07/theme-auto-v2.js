(()=>{const safePreferences = {getItem:k=>{try{return FCCStore.getItem(k)}catch{return null}},setItem:(k,v)=>{try{FCCStore.setItem(k,v).catch(()=>{})}catch{}},removeItem:k=>{try{FCCStore.removeItem(k).catch(()=>{})}catch{}}};
(()=>{
  ...
  function getOverride(){try{const x=JSON.parse(safePreferences.getItem(KEY)||'null');if(x&&(x.theme==='light'||x.theme==='dark')&&+x.until>Date.now())return x}catch(e){}safePreferences.removeItem(KEY);return null}
  function setOverride(theme){safePreferences.setItem(KEY,JSON.stringify({theme,until:boundary()}));status()}
  ...
  let timer=0;function schedule(){clearTimeout(timer);timer=setTimeout(()=>{safePreferences.removeItem(KEY);sync()},Math.max(1000,boundary()-Date.now()+1200))}
