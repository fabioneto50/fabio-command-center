...
(function(root,factory){
 ...
 return function createStore({storage,crypto,locks=null,emit=()=>{}}){
  ...
  function setItem(k,v){
   v=String(v);
   if(Object.hasOwn(PRIVATE,k)){requireOpen();if(transactionBusy)throw new Error('Importação em curso. Aguarda antes de editar.');records[k]=v;return enqueue({...records});}
   if(Object.hasOwn(PUBLIC,k)||EPHEMERAL.has(k)){if(transactionBusy&&Object.hasOwn(PUBLIC,k))throw new Error('Importação em curso.');storage.setItem(k,v);return Promise.resolve();}
