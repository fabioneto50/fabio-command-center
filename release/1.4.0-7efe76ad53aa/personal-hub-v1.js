(()=>{
  ...
  const CHILDREN=[
    {page:'expenses',code:'€',title:'Despesas',desc:'Registo, análise, recorrências e cópias de segurança.'},
    {page:'emergency',code:'EM',title:'Emergência',desc:'Preparação, inventário, família, cenários e notas.'},
    {page:'comms',code:'RF',title:'Comunicações',desc:'Meshtastic, rádio, telemetria, sensores e testes.'},
    {page:'garage',code:'GX',title:'Veículos',desc:'Veículos, manutenção, custos, documentos e acessórios.'},
    {page:'research',code:'RX',title:'Investigação',desc:'Biblioteca, pesquisa de evidência, referências e guidelines.'}
  ];
  ...
  function install(){addStyles();ensurePage();ensureNav();simplifyHome();renderCards();CHILDREN.forEach(x=>window.FCCNavigation?.setParent(x.page,'personal'));const brand=document.querySelector('.brand p');if(brand)brand.textContent='Clínica · Pessoal · 1.4.0'}
  install();
