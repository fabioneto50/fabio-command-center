(()=>{
  ...
  function analyseImage(img){
    ...
    rows.push(w>=1200&&h>=700?{level:'good',icon:'✓',title:'Resolução',text:`${w} × ${h}px · adequada para tentativa de digitalização.`}:{level:'warn',icon:'!',title:'Resolução',text:`${w} × ${h}px · tenta fotografar com maior resolução e sem zoom digital.`});
    ...
    try{
      ...
      rows.push(sd>=28?{level:'good',icon:'✓',title:'Contraste',text:'Contraste global suficiente para tentativa de extração do traçado.'}:{level:'warn',icon:'!',title:'Contraste',text:'Contraste baixo; aproxima a câmara e garante foco nas grelhas e traçados.'});
    }catch(e){}
    rows.push({level:'good',icon:'i',title:'Próximo passo',text:'O analisador abaixo tenta digitalizar o traçado localmente. Só apresenta medições se grelha, calibração e extração passarem os controlos de qualidade.'});
    return rows;
  }
  ...
  function install(){
    ...
    const card=document.createElement('div');card.className='ecgp-card';card.innerHTML=`<div class="ecgp-head"><div><h3>ECG por fotografia</h3><p>Carrega ou fotografa o ECG. A app verifica primeiro a qualidade da imagem antes de tentar digitalizar o traçado.</p></div><span class="badge warn">Imagem → sinal</span></div><div class="ecgp-grid"><div><div class="ecgp-upload"><b style="font-size:var(--fcc-caption,.8125rem)">Fotografia do ECG</b><div class="tiny" style="margin-top:4px">Ideal: 12 derivações completas, imagem horizontal, sem reflexos, foco nítido e grelha visível.</div><input id="ecgPhotoInput" type="file" accept="image/*" capture="environment"><div class="ecgp-actions"><button type="button" class="btn" id="ecgPhotoClear">Remover imagem</button><button type="button" class="btn primary" id="ecgStructuredJump">Analisador estruturado ↓</button></div></div><div id="ecgPhotoQuality" class="ecgp-quality"></div></div><div><div id="ecgPhotoPreview" class="ecgp-preview"><div class="ecgp-placeholder">Fotografa ou escolhe um ECG de 12 derivações.</div></div><div class="ecgp-note"><b>Privacidade e segurança:</b> a fotografia é processada localmente no browser e não é enviada para um servidor por este módulo. A análise automática abaixo é experimental e bloqueia resultados quando a qualidade/confiança é insuficiente; confirma sempre no ECG original.</div></div></div>`;
