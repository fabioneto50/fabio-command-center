(()=>{
  ...
  function routeBlock(r){
    return `<section class="ccd-doc-route cuf2213-route">
      <div class="ccd-doc-routehead"><h4 class="cuf-route-title">${esc(r.route)}</h4><span>${esc(r.dose)} · pág. ${r.page}</span></div>
      <div class="ccd-doc-grid"><h5 class="ccd-doc-section">Apresentação</h5>${field('Dosagem',r.dose)}${field('Forma farmacêutica',r.form)}${field('Fonte','INF.2213.00')}</div>
      <div class="ccd-doc-grid"><h5 class="ccd-doc-section">Reconstituição</h5>${field('Especificações',r.reconstitution)}${field('Estabilidade',r.reconstitution_stability)}</div>
      <div class="ccd-doc-grid"><h5 class="ccd-doc-section">Diluição</h5>${field('Especificações',r.dilution)}${field('Estabilidade',r.dilution_stability)}</div>
  ...
  function currentDetails(drug,rows){
    return `<div class="cuf-current-note"><b>Documento fornecido para antibióticos: INF.2213.00 · 28/08/2026</b><span>Transcrição conferida nas oito páginas do documento, não uma nova validação clínica. A apresentação, via e fabricante condicionam a preparação. Não generalizar condições de uma apresentação a outra.</span></div>
      <div class="fcc-route-grid">${rows.map(routeBlock).join('')}</div>
      ${/ceftriaxona|teicoplanina/i.test(drug)?'<div class="advice"><strong>Confirmar formulação e contexto na fonte primária atual</strong><p>A redação específica de diluição/via do guia foi preservada, sem correção clínica automática. Rever com o farmacêutico e protocolo institucional antes de usar.</p></div>':''}
  ...
  function makeCard(drug,rows){
    ...
    a.innerHTML=`<div class="ccd-doc-top"><div><h3>${esc(drug)}</h3><p><b>INF.2213.00</b> · Antibacterianos · ${rows.length} via${rows.length===1?'':'s'}/apresentação</p></div><div class="ccd-doc-routebadges">${[...new Set(rows.map(r=>r.route.split('(')[0].trim()))].map(x=>`<span class="badge">${esc(x)}</span>`).join('')}<span class="badge good">Documento fornecido</span></div></div><details class="ccd-doc-details"><summary>Ver reconstituição, diluição e administração</summary>${currentDetails(drug,rows)}</details>`;
    return a;
  }
  ...
  function updateSourceUI(){
    ...
    if(banner&&banner.dataset.cufCurrent!=='1'){
      ...
      banner.innerHTML='<b>Fontes institucionais integradas:</b> <strong>INF.2213.00</strong> (documento fornecido, versão indicada em 28/08/2026; transcrição conferida nas oito páginas, sem nova validação clínica) para reconstituição/diluição/administração de antibióticos e <strong>INF.1030.11</strong> para estabilidade após abertura. O antigo DILUIÇÕES.xlsx permanece apenas como fallback quando não existe entrada nestes documentos.';
    }
    const meta=document.querySelector('#clin-perf .ccd-meta');
    if(meta){
      let b=document.getElementById('cuf2213Badge');if(!b){b=document.createElement('span');b.id='cuf2213Badge';b.className='badge good';meta.appendChild(b)}b.textContent='INF.2213.00 · documento de 28/08/2026';
  ...
  function addStyles(){
    ...
    const s=document.createElement('style');s.id='cuf-dil-v6-style';s.textContent=`
      .cuf-current-note{border:1px solid rgba(114,227,167,.28);background:rgba(114,227,167,.07);border-radius:11px;padding:9px 10px;margin:9px 0}.cuf-current-note b{display:block;color:var(--tactical);font-size:var(--fcc-caption,.8125rem)}.cuf-current-note span{display:block;color:var(--muted);font-size:var(--fcc-caption,.8125rem);line-height:1.45;margin-top:4px}
  ...
  function enhance(){
    ...
    try{
      ...
      for(const card of allCards){
        // Additional institutional records also count as present on later enhancement passes.
        ...
        seen.add(key);card.style.removeProperty('display');delete card.dataset.cufSuperseded;card.dataset.cufCurrent='1';
        h3.textContent=drug;
        ...
        const badges=card.querySelector('.ccd-doc-routebadges');if(badges&&!badges.querySelector('.cuf-current-badge'))badges.insertAdjacentHTML('beforeend','<span class="badge good cuf-current-badge">Documento fornecido</span>');
