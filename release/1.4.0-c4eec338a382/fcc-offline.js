...
(()=>{
 ...
 const pageBuild=()=>new URL(window.FCC_ASSET_BASE||'./',location.href).pathname.match(/\/release\/(1\.4\.0-[a-f0-9]{12})\//)?.[1]||'';
 function send(type,payload={}){return new Promise((resolve,reject)=>{
  // A waiting worker belongs to another release and must not report readiness for this page.
  const target=navigator.serviceWorker?.controller||registration?.active;
  if(!target)return reject(new Error('A preparação offline ainda não está pronta.'));
  const channel=new MessageChannel();let finished=false;
  const finish=(error,data)=>{if(finished)return;finished=true;clearTimeout(timer);channel.port1.close();if(error)reject(error);else resolve(data);};
  const timer=setTimeout(()=>finish(new Error('Tempo de espera excedido. Tenta novamente.')),type==='CACHE_PACKAGE'?180000:8000);
  channel.port1.onmessage=e=>{if(e.data?.progress!==undefined){const status=document.getElementById('fccOfflineProgress');if(status)status.textContent='Preparar pacote: '+e.data.progress+' / '+e.data.total;return;}finish(e.data?.error?new Error(e.data.error):null,e.data);};
  try{target.postMessage({type,...payload,expectedBuild:pageBuild()},[channel.port2]);}catch(e){finish(e);}
 });}
 const size=n=>(n/1024/1024).toFixed(2)+' MB';
 async function refresh(){
  const box=document.getElementById('fccOfflinePackages');if(!box)return;
  try{const data=await send('STATUS');if(pageBuild()&&data.build!==pageBuild())throw new Error('Outra versão foi ativada. Guarda o trabalho em curso e recarrega a página para verificar os pacotes da nova versão.');manifest=data.manifest;box.innerHTML=Object.entries(data.packages).map(([key,p])=>`<div class="item"><div><strong>${esc(p.label)}</strong><p>${p.complete?'Disponível offline nesta versão':'Ainda não preparado'} · ${esc(size(p.bytes))} · ${p.files} recursos</p></div><button class="btn" data-pack="${esc(key)}" ${p.complete?'disabled':''}>${p.complete?'Preparado':'Preparar'}</button></div>`).join('');box.querySelectorAll('[data-pack]').forEach(b=>b.onclick=()=>U.busy(b,async()=>{await send('CACHE_PACKAGE',{package:b.dataset.pack});await refresh();U.notify('Pacote verificado e disponível offline.');}));const info=document.getElementById('fccOfflineBuild');if(info)info.textContent='Recursos offline: '+data.build+'. Material externo e pesquisa de artigos necessitam de ligação.';}
