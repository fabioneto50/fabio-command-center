...
(()=>{
 ...
 function show(name,{route=true,focus=true}={}){
  ...
  clearTimeout(timer);if(route)syncRoute();current=record;detail(record);if(route)FCCNavigation.replaceDetail(id(record),false);if(focus)document.querySelector('#med4Results h3')?.focus({preventScroll:true});
