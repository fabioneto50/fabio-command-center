...
(()=>{
 ...
 function article(x){const image=safe(x.image),link=safe(x.articleUrl||x.url);return `<a class="home-news-item" href="${esc(link)}" target="_blank" rel="noopener noreferrer"><div class="home-news-media">${image?`<img src="${esc(image)}" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer">`:'<div class="home-news-media-fallback" aria-hidden="true">Notícia</div>'}</div><div class="home-news-body"><p class="home-news-kicker">${esc(x.source||'Fonte')} · ${esc(date(x.publishedAt))}</p><h4>${esc(x.title)}</h4><p>${esc(String(x.description||'').slice(0,900))}</p><span>Ler na fonte original ↗</span></div></a>`;}
 function render(kind){
  ...
  root.querySelectorAll('img').forEach(img=>img.onerror=()=>{const p=document.createElement('div');p.className='home-news-media-fallback';p.textContent='Imagem indisponível';img.replaceWith(p);});
