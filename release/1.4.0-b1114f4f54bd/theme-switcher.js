...
(()=>{
  ...
  const bootTheme=()=>{try{const x=JSON.parse(safePreferences.getItem('fcc-theme-manual-until-v2')||'null');if(x&&valid(x.theme)&&+x.until>Date.now())return x.theme}catch(e){}return autoTheme()};
  let theme=bootTheme();
  ...
  function addStyles(){
    ...
    st.textContent=`
      ...
      .fcc-theme-toggle button{border:0;background:transparent;color:var(--muted);padding:7px 10px;border-radius:9px;font-size:var(--fcc-caption,.8125rem);font-weight:800;cursor:pointer}
  ...
  function apply(next,persist=true){
    ...
    if(persist)safePreferences.setItem(KEY,theme);
