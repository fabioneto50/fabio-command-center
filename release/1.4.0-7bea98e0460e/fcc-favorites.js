...
(()=>{
 ...
 const PRIVATE_KEY='fcc-personal-favorites-v1';
 const key=x=>[x.page,x.sub||'',x.ref||''].join('/');
 const privateSafe=x=>!!x&&['expenses','emergency','comms','garage','research'].includes(x.page)&&!x.ref&&(!(x.sub||'')||(x.page==='emergency'&&x.sub==='em-notes'))&&typeof x.title==='string'&&x.title.length<=160;
 function personalList(){if(!FCCAccess.isUnlocked())return [];const p=read(PRIVATE_KEY);return (Array.isArray(p.favorites)?p.favorites:[]).filter(privateSafe).map(x=>({...x}));}
 function refreshFavorites(){renderHome();renderPage();document.dispatchEvent(new CustomEvent('fcc-favorites-change'));}
 async function togglePersonal(item){
  if(!FCCAccess.isUnlocked())return FCCAccess.run(()=>togglePersonal(item));
  const rows=personalList(),i=rows.findIndex(x=>key(x)===key(item));
  if(i<0)rows.push({page:item.page,sub:item.sub||'',ref:'',title:item.title});else rows.splice(i,1);
  try{await S.setItem(PRIVATE_KEY,JSON.stringify({version:1,favorites:rows}));refreshFavorites();U.notify(i<0?'Adicionado aos favoritos pessoais.':'Removido dos favoritos pessoais.');return true;}
  catch(e){U.notify(e.message,'error');return false;}
 }
 ...
 function list(){return [...data.favorites.filter(safe).map(x=>({...x})),...personalList()];}
 function has(item){return privateSafe(item)?personalList().some(x=>key(x)===key(item)):safe(item)&&data.favorites.some(x=>key(x)===key(item));}
 async function toggle(item){
  if(privateSafe(item))return togglePersonal(item);
 ...
 function renderPage(){
  ...
  let lock=page.querySelector('.fcc-favorites-privacy');
  if(!lock){lock=document.createElement('div');lock.className='fcc-favorites-privacy';lock.innerHTML='<p>Os favoritos pessoais estão protegidos pelo cofre.</p><button class="btn" type="button">Desbloquear favoritos pessoais</button>';grid.before(lock);lock.querySelector('button').onclick=()=>FCCAccess.run(renderPage);}
  lock.hidden=FCCAccess.isUnlocked();
 ...
 function installPage(){
  ...
  const page=document.createElement('section');page.id='page-favorites';page.className='page';page.innerHTML=`<div class="pagehead"><div><h2 tabindex="-1">Favoritos</h2><p>Os teus módulos, fichas clínicas e atalhos pessoais.</p></div></div><section class="fcc-area-hub"><label class="fcc-area-search">${FCCAreas.icon('search')}<span class="fcc-sr-only">Pesquisar nos favoritos</span><input type="search" maxlength="180" autocomplete="off" placeholder="Pesquisar nos favoritos"></label><div class="fcc-area-caption"><h3>Guardados por ti</h3><span role="status" aria-live="polite"></span></div><div class="fcc-area-grid" role="list"></div></section>`;
 ...
 window.FCCFavorites=Object.freeze({toggle,recent,renderHome,settings,list,has,renderPage,installPage});document.addEventListener('fcc-private-refresh',()=>{if(!FCCAccess.isUnlocked()){const q=document.querySelector('#page-favorites input');if(q)q.value='';}data=read(KEY);data.favorites=Array.isArray(data.favorites)?data.favorites:[];data.recent=Array.isArray(data.recent)?data.recent:[];renderHome();renderPage();document.dispatchEvent(new CustomEvent("fcc-favorites-change"));});installPage();settings();renderHome();
})();
