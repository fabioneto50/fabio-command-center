(()=>{
  ...
  function addStyles(){
    ...
    const s=document.createElement('style');s.id='fcc-recurring-ui-style';s.textContent=`
      .exr-kpis{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px}.exr-kpi{border:1px solid var(--line);background:var(--panel);border-radius:15px;padding:13px}.exr-kpi small{display:block;color:var(--muted);font-size:var(--fcc-caption,.8125rem);text-transform:uppercase;letter-spacing:.07em}.exr-kpi strong{display:block;font-size:21px;margin-top:5px}.exr-kpi span{display:block;color:var(--muted);font-size:var(--fcc-caption,.8125rem);margin-top:3px}
      .exr-form{display:grid;grid-template-columns:repeat(4,1fr);gap:9px;margin-top:10px}.exr-wide{grid-column:span 2}.exr-plan{border:1px solid var(--line);background:var(--panel-2);border-radius:14px;padding:12px;display:grid;gap:9px}.exr-plan-top{display:flex;gap:10px;align-items:flex-start}.exr-plan-top h4{margin:0;font-size:var(--fcc-caption,.8125rem)}.exr-plan-top p{margin:3px 0 0;color:var(--muted);font-size:var(--fcc-caption,.8125rem)}.exr-plan .spacer{flex:1}.exr-meta{display:flex;gap:6px;flex-wrap:wrap}.exr-chip{font-size:var(--fcc-caption,.8125rem);padding:4px 7px;border:1px solid var(--line);border-radius:999px;background:var(--panel);color:var(--muted)}.exr-chip.active{border-color:rgba(114,227,167,.35);color:var(--good)}.exr-chip.paused{border-color:rgba(242,185,94,.35);color:var(--warn)}.exr-chip.cancelled{border-color:rgba(255,123,134,.35);color:var(--danger)}
      .exr-actions{display:flex;gap:6px;flex-wrap:wrap}.exr-empty{padding:18px;text-align:center;border:1px dashed var(--line);border-radius:13px;color:var(--muted);font-size:var(--fcc-caption,.8125rem)}.exr-note{margin-top:6px;color:var(--muted);font-size:var(--fcc-caption,.8125rem);line-height:1.5}.exr-day-note{display:block;margin-top:3px;color:var(--muted);font-size:var(--fcc-caption,.8125rem);min-height:12px}
  ...
  function save(){
    ...
    if(res.added)window.FCCExpenseRefresh?.();
  }
  function edit(key){
  ...
  function status(key,next){
    ...
    const res=E.setStatus(key,next);render();toastSafe(next==='paused'?'Recorrente pausado':next==='cancelled'?'Recorrente cancelado':'Recorrente reativado');if(res?.added)window.FCCExpenseRefresh?.();
  }
  ...
  document.addEventListener('fcc-private-refresh',()=>{editing='';render()});
