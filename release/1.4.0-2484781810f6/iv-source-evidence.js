(()=>{
  ...
  function css(){
    ...
    const st=document.createElement('style');st.id='ivsrc-style';st.textContent=`
      ...
      .ivsrc-head{display:flex;gap:10px;align-items:flex-start;justify-content:space-between;padding:12px 13px;border-bottom:1px solid rgba(113,158,190,.14)}.ivsrc-head h4{margin:0;font-size:var(--fcc-caption,.8125rem)}.ivsrc-head p{margin:3px 0 0;color:var(--muted);font-size:var(--fcc-caption,.8125rem);line-height:1.45}.ivsrc-head .badge{white-space:nowrap}
      .ivsrc-summary{padding:11px 13px;font-size:var(--fcc-caption,.8125rem);line-height:1.5;border-bottom:1px solid rgba(113,158,190,.12)}
      .ivsrc-rows{display:grid}.ivsrc-row{display:grid;grid-template-columns:minmax(160px,1.15fr) minmax(150px,.85fr) 28px;gap:10px;align-items:center;padding:10px 13px;border-bottom:1px solid rgba(113,158,190,.10)}.ivsrc-row:last-child{border-bottom:0}.ivsrc-name b{display:block;font-size:var(--fcc-caption,.8125rem)}.ivsrc-name span{display:block;color:var(--muted);font-size:var(--fcc-caption,.8125rem);margin-top:2px}.ivsrc-state{display:inline-flex;width:max-content;max-width:100%;align-items:center;gap:6px;border-radius:999px;padding:5px 8px;font-size:var(--fcc-caption,.8125rem);font-weight:850;border:1px solid var(--line)}.ivsrc-state:before{content:'';width:7px;height:7px;border-radius:50%;background:currentColor}.ivsrc-compatible{color:#74dfb8;background:rgba(59,209,155,.07);border-color:rgba(59,209,155,.22)}.ivsrc-incompatible{color:#ff929b;background:rgba(255,110,121,.07);border-color:rgba(255,110,121,.22)}.ivsrc-conditional{color:#f3ca79;background:rgba(244,187,85,.07);border-color:rgba(244,187,85,.22)}.ivsrc-unknown{color:#8ca4b4;background:rgba(113,158,190,.05);border-color:rgba(113,158,190,.16)}.ivsrc-link{display:grid;place-items:center;width:28px;height:28px;border-radius:9px;border:1px solid rgba(113,158,190,.18);text-decoration:none;color:#8ddfff;background:rgba(8,22,34,.65)}
      .ivsrc-note{padding:9px 13px;color:var(--muted);font-size:var(--fcc-caption,.8125rem);line-height:1.5;background:rgba(113,158,190,.025)}
  ...
  function summary(rows){
    ...
    return `<b>Sem resultado integrado.</b> As ligações apresentadas não foram classificadas individualmente nesta versão. O resultado documental do par não é atribuído automaticamente a cada fonte; confirmar o estudo e as condições. Isto não equivale a incompatibilidade.`;
  }
  ...
  function decorate(){
    ...
    anchors.forEach(a=>{
      ...
      const s='unknown'; // No per-source attribution without a claim-specific verified record.
  ...
  function init(){
    ...
    let busy=false;
    const obs=new MutationObserver(mutations=>{if(busy||mutations.every(m=>m.target.closest?.('.ivsrc-panel')||[...m.addedNodes,...m.removedNodes].every(n=>n.nodeType===1&&n.classList.contains('ivsrc-panel'))))return;busy=true;requestAnimationFrame(()=>{decorate();busy=false})});
