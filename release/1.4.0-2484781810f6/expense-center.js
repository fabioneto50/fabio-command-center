(()=>{
  ...
  function loadState(){
    try{
      const raw=JSON.parse(FCCStore.getItem(KEY)||'null');
  ...
  let data=loadState();
  function save(){if(window.FCCRecurringEngine?.saveIncoming)window.FCCRecurringEngine.saveIncoming(data,{source:'expense-center'});else FCCStore.setItem(KEY,JSON.stringify(data));renderAll()}
  ...
  function addStyles(){
    ...
    const st=document.createElement('style');st.id='fcc-expense-style';st.textContent=`
      ...
      .exp-natural{position:relative}.exp-natural input{font-size:16px;padding:13px 14px}.exp-hint{margin-top:5px;font-size:var(--fcc-caption,.8125rem);color:var(--muted)}
      .exp-suggestion{margin-top:10px;border:1px solid rgba(79,198,164,.25);background:rgba(79,198,164,.06);border-radius:14px;padding:11px;display:flex;gap:8px;align-items:center;justify-content:space-between;flex-wrap:wrap}.exp-suggestion b{font-size:var(--fcc-caption,.8125rem)}.exp-suggestion span{font-size:var(--fcc-caption,.8125rem);color:var(--muted)}
      .exp-kpis{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px}.exp-kpi{border:1px solid var(--line);border-radius:15px;background:var(--panel);padding:13px}.exp-kpi small{display:block;color:var(--muted);font-size:var(--fcc-caption,.8125rem);text-transform:uppercase;letter-spacing:.08em}.exp-kpi strong{display:block;margin-top:5px;font-size:22px;letter-spacing:-.03em}.exp-kpi span{display:block;color:var(--muted);font-size:var(--fcc-caption,.8125rem);margin-top:3px}
      .exp-bars{display:grid;gap:7px}.exp-bar{display:grid;grid-template-columns:minmax(110px,.8fr) minmax(120px,2fr) auto;gap:9px;align-items:center}.exp-bar label{display:block;font-size:var(--fcc-caption,.8125rem);color:var(--text)}.exp-track{height:8px;background:var(--panel-3);border-radius:999px;overflow:hidden;border:1px solid var(--line)}.exp-fill{height:100%;background:linear-gradient(90deg,var(--clinical),var(--tactical));border-radius:999px}.exp-bar b{font-size:var(--fcc-caption,.8125rem)}
      .exp-table{width:100%;border-collapse:collapse}.exp-table th,.exp-table td{padding:9px;border-bottom:1px solid var(--line);font-size:var(--fcc-caption,.8125rem)}.exp-table th{color:var(--muted);font-weight:700}.exp-cat-chip{display:inline-flex;padding:4px 7px;border-radius:999px;background:var(--clinical-soft);border:1px solid rgba(98,212,255,.18);font-size:var(--fcc-caption,.8125rem)}
      ...
      .exp-budget-row{display:grid;grid-template-columns:minmax(120px,1fr) 120px 1.6fr auto;gap:8px;align-items:center;border-bottom:1px solid var(--line);padding:9px 0}.exp-budget-row:last-child{border:0}.exp-budget-row strong{font-size:var(--fcc-caption,.8125rem)}.exp-budget-row input{padding:7px}.exp-budget-progress{height:8px;border-radius:999px;background:var(--panel-3);overflow:hidden;border:1px solid var(--line)}.exp-budget-progress>span{display:block;height:100%;background:var(--clinical)}
      .exp-recurring{display:grid;grid-template-columns:1fr auto auto;gap:8px;align-items:center;border:1px solid var(--line);border-radius:12px;padding:9px;background:var(--panel-2)}.exp-recurring strong{font-size:var(--fcc-caption,.8125rem)}.exp-recurring span{font-size:var(--fcc-caption,.8125rem);color:var(--muted)}
      .exp-filter-row{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}.exp-empty{padding:18px;text-align:center;color:var(--muted);font-size:var(--fcc-caption,.8125rem);border:1px dashed var(--line);border-radius:13px}
  ...
  function ensurePage(){
    ...
    page.innerHTML=`
      ...
      ${section('exp-reports','Relatórios / Exportação',`
        <div class="card half"><h3>Exportar dados</h3><p>A cópia JSON completa é cifrada. O CSV é legível e deve ser guardado em local protegido.</p><div class="actions"><button class="btn primary" id="expExportJson">Cópia completa cifrada</button><button class="btn" id="expExportCsv">Exportar CSV</button></div></div>
  ...
  function removeExpense(id){FCCRecurringEngine.removeMovement(id);data=loadState();renderAll();toastSafe('Despesa eliminada; não será recriada automaticamente.');}
  ...
  function exportJson(){return FCCBackup.exportAll();}
  function exportCsv(){
  ...
  function importJson(e){const f=e.target.files?.[0];e.target.value='';return FCCBackup.importExpenses(f).catch(err=>FCCUI.notify(err.message,'error'));}
  ...
  function init(){
    addStyles();ensurePage();initControls();renderAll();
    setTimeout(()=>{
      try{
        const saved=JSON.parse(FCCStore.getItem(ORDER_KEY)||'{}');
  ...
  window.openExpensesMenu=()=>window.openCategoryMenu('expenses');
  document.addEventListener('fcc-private-refresh',()=>{data=loadState();renderAll();});
  window.addEventListener('fcc-expenses-changed',()=>{data=loadState();renderAll();});
  window.FCCExpenseRefresh=()=>{data=loadState();renderAll();};
})();