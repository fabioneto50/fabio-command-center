(()=>{
  ...
  function currentStandard(drug,route){
  ...
  function hasDilutionData(x){return !!(x.routes.some(r=>currentStandard(x.drug,r.route))||x.routes.some(r=>r.reconstitution_solvent||r.reconstitution_volume||r.diluent||r.dilution_volume))}
  ...
  function render(){
    ...
    grid.innerHTML=rows.map(x=>{
      const routeNames=[...new Set(x.routes.map(r=>r.route).filter(Boolean))];
      return `<article class="card full ccd-doc-card"><div class="ccd-doc-top"><div><h3>${esc(x.drug)}</h3><p>${x.brand?`<b>${esc(x.brand)}</b> · `:''}${esc(x.category)}${x.article?` · ${esc(x.article)}`:''}</p></div><div class="ccd-doc-routebadges">${routeNames.map(r=>`<span class="badge">${esc(r)}</span>`).join('')}</div></div><details class="ccd-doc-details"><summary>Ver reconstituição, diluição e administração · ${x.routes.length} registo${x.routes.length===1?'':'s'}</summary><div class="fcc-route-grid">${x.routes.map(routeBlock).join('')}</div><div class="ccd-doc-source">Fonte importada: ${SOURCE_LABEL}. Conteúdo documental, não revalidado linha a linha em 2026. Confirmar RCM/SmPC e protocolo institucional antes de utilização clínica.</div></details></article>`;
    }).join('');
  }

  function addStyles(){
    if(document.getElementById('ccd-doc-v4-style'))return;
    const s=document.createElement('style');s.id='ccd-doc-v4-style';s.textContent=`.ccd-doc-card{padding:13px!important}.ccd-doc-top{display:flex;justify-content:space-between;align-items:flex-start;gap:12px}.ccd-doc-top h3{margin:0 0 4px!important}.ccd-doc-top p b{color:var(--text)}.ccd-doc-routebadges{display:flex;gap:4px;flex-wrap:wrap;justify-content:flex-end;max-width:46%}.ccd-doc-details{margin-top:9px;border-top:1px solid var(--line);padding-top:8px}.ccd-doc-details>summary{cursor:pointer;color:var(--clinical);font-size:var(--fcc-caption,.8125rem);font-weight:800;list-style:none}.ccd-doc-details>summary::-webkit-details-marker{display:none}.ccd-doc-details>summary:after{content:' +';color:var(--muted)}.ccd-doc-details[open]>summary:after{content:' −'}.ccd-doc-route{margin-top:9px;border:1px solid var(--line);border-radius:12px;padding:9px;background:var(--panel-2)}.ccd-doc-routehead{display:flex;justify-content:space-between;gap:8px;align-items:center;margin-bottom:7px}.ccd-doc-routehead b{font-size:var(--fcc-caption,.8125rem)}.ccd-doc-routehead span{font-size:var(--fcc-caption,.8125rem);color:var(--muted)}.ccd-doc-grid{display:grid;grid-template-columns:90px repeat(3,minmax(0,1fr));gap:6px;margin-top:5px}.ccd-doc-grid-wide{grid-template-columns:repeat(3,minmax(0,1fr))}.ccd-doc-section{font-size:var(--fcc-caption,.8125rem);font-weight:900;text-transform:uppercase;letter-spacing:.08em;color:var(--muted);display:flex;align-items:center}.ccd-doc-field{border:1px solid var(--line);border-radius:9px;padding:7px;background:var(--panel)}.ccd-doc-field small{display:block;font-size:var(--fcc-caption,.8125rem);color:var(--muted);margin-bottom:3px}.ccd-doc-field div{font-size:var(--fcc-caption,.8125rem);line-height:1.45}.ccd-doc-source{margin-top:8px;font-size:var(--fcc-caption,.8125rem);line-height:1.5;color:var(--muted)}.ccd-ashp .tiny{margin-top:4px;line-height:1.45}.ccd-doc-source-banner{border-color:rgba(168,156,255,.25)!important;background:rgba(168,156,255,.06)!important}.ccd-doc-source-banner b{color:var(--violet)}html[data-fcc-theme="light"] .ccd-doc-route,html[data-fcc-theme="light"] .ccd-doc-field{background:#fff!important}@media(max-width:760px){.ccd-doc-top{display:block}.ccd-doc-routebadges{max-width:none;justify-content:flex-start;margin-top:7px}.ccd-doc-grid,.ccd-doc-grid-wide{grid-template-columns:1fr}.ccd-doc-section{margin-top:3px}}`;document.head.appendChild(s);
  }
