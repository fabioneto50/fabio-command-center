...
(()=>{
 ...
 function render(kind){
  ...
  if(rows.length){root.innerHTML=`<div class="home-news-visible">${rows.slice(positions[kind]*size,(positions[kind]+1)*size).map(article).join('')}</div><nav class="home-news-dots" aria-label="Páginas de notícias ${kind==='health'?'de saúde':'do mundo'}">${Array.from({length:pages},(_,i)=>`<button class="btn home-news-dot" type="button" data-page="${i}" aria-label="Página ${i+1} de ${pages}" ${i===positions[kind]?'aria-current="page"':''}><span>${i+1}</span></button>`).join('')}</nav>`;root.querySelectorAll('[data-page]').forEach(b=>b.onclick=()=>{positions[kind]=+b.dataset.page;render(kind);root.querySelector(`[data-page="${positions[kind]}"]`)?.focus();});}
