...
(()=>{
 ...
 function render(){
  ...
  document.querySelectorAll('#page-emergency,#page-comms,#page-garage,#page-research,#page-expenses').forEach(root=>{if(!root.querySelector(':scope > .fcc-lock-area')){const button=document.createElement('button');button.className='btn fcc-lock-area';button.type='button';button.textContent='Bloquear área pessoal';button.onclick=()=>FCCAccess.lock();root.prepend(button);}});
