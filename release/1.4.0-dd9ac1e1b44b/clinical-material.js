(()=>{
  ...
  function ensureStyles(){
    ...
    st.textContent=`
      ...
      .material-toolbar .spacer{flex:1}.material-source-chip{font-size:var(--fcc-caption,.8125rem);letter-spacing:.08em;text-transform:uppercase;color:#92dfff;border:1px solid rgba(92,202,255,.25);background:rgba(92,202,255,.07);border-radius:999px;padding:6px 9px}
      ...
      .material-frame-head h3{margin:0;font-size:14px}.material-frame-head p{margin:3px 0 0;font-size:var(--fcc-caption,.8125rem);color:var(--muted)}
      .material-frame{display:block;width:100%;height:68vh;border:0;background:#fff}
      .material-help{padding:10px 12px;border:1px solid rgba(244,187,85,.20);background:rgba(244,187,85,.06);border-radius:13px;color:#d8bf85;font-size:var(--fcc-caption,.8125rem);line-height:1.5}
  ...
  function ensureModule(){
    ...
    sub.innerHTML=`
      <div class="material-shell">
        ...
        <div class="material-frame-card">
          ...
          <iframe id="materialStockFrame" class="material-frame" src="about:blank" title="Stock de material" loading="lazy" referrerpolicy="no-referrer"></iframe>
        </div>
      </div>`;
    ...
    if(input) input.value=FCCStore.getItem(NOTES_KEY)||'';
    document.getElementById('materialNotesSave')?.addEventListener('click',()=>{
      const text=input?.value||'';if(!FCCStore.status().unlocked)return FCCAccess.run(async()=>{if(input)input.value=text;await FCCStore.setItem(NOTES_KEY,text);FCCUI.notify('Notas guardadas no cofre.');});FCCStore.setItem(NOTES_KEY,text).catch(e=>FCCUI.notify(e.message,'error'));
...
document.addEventListener('fcc-private-refresh',()=>{const el=document.getElementById('materialNotesInput');if(el)el.value=FCCStore.getItem('fcc-clinical-material-notes-v1')||'';});
