(()=>{
  ...
  const freqLabel=f=>({weekly:'Semanal',fortnightly:'Quinzenal',monthly:'Mensal',quarterly:'Trimestral',yearly:'Anual'})[f]||f;
  ...
  function addStyles(){
    ...
    const s=document.createElement('style');s.id='fcc-recurring-ui-style';s.textContent=`
      ...
      .exr-actions{display:flex;gap:6px;flex-wrap:wrap}.exr-empty{padding:18px;text-align:center;border:1px dashed var(--line);border-radius:13px;color:var(--muted);font-size:9px}.exr-note{margin-top:6px;color:var(--muted);font-size:8px;line-height:1.5}
    ...
    host.innerHTML=`<div class="exp-shell">
      ...
      <div class="card full">
        ...
        <div class="exr-form">
          ...
          <label>Frequência<select id="exrFrequency"><option value="monthly">Mensal</option><option value="weekly">Semanal</option><option value="fortnightly">Quinzenal</option><option value="quarterly">Trimestral</option><option value="yearly">Anual</option></select></label>
          <label>Data de início<input id="exrStart" type="date"></label>
          <label>Dia de cobrança<input id="exrDay" type="number" min="1" max="31" placeholder="1–31"></label>
        ...
        <div class="exr-note">Ao marcar uma despesa normal como “recorrente / subscrição”, é criado automaticamente um plano mensal. Podes depois ajustar frequência, data e link de cancelamento aqui.</div>
      </div>
