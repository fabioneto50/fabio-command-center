(()=>{
  ...
  function row(r){
    ...
    return `<div class="cuf-stab-row"><div class="cuf-stab-main"><b>${esc(r.designation)}</b>${r.brand?`<span>${esc(r.brand)}</span>`:''}</div><div class="cuf-stab-code">${code?`Código CUF <b>${esc(code)}</b>`:'Código CUF não indicado'}</div><div><small>Forma</small>${esc(r.form||'—')}</div><div><small>Estabilidade após abertura</small><b>${esc(r.stability||'—')}</b></div>${r.observations?`<div class="cuf-stab-obs"><small>Observações</small>${esc(r.observations)}</div>`:''}</div>`;
  }
  function panel(rows){
